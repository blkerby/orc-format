...
impl<'a> FloatData<'a> {
    ...
    pub fn write(&mut self, x: Option<f32>) {
        match x {
            Some(xv) => {
                self.present.write(true);
                self.data.write_f32::<LittleEndian>(xv).unwrap();
            }
            None => { 
                self.present.write(false); 
            }
        }
        self.stripe_stats.update(x.map(|y| y as f64));
    }
}

impl<'a> BaseData<'a> for FloatData<'a> {
