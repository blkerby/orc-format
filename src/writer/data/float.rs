...
pub struct FloatData {
    ...
    streams: FloatDataStreams,
    stripe_stats: DoubleStatistics,
    row_group_stats: DoubleStatistics,
    row_group_position: FloatDataPosition,
    row_index_entries: Vec<FloatRowIndexEntry>,
    config: Config,
}

struct FloatDataStreams {
    present: BooleanRLE,
    data: CompressionStream,
}

#[derive(Copy, Clone)]
struct FloatDataPosition {
    present: BooleanRLEPosition,
    data: CompressionStreamPosition,
}

struct FloatRowIndexEntry {
    position: FloatDataPosition,
    stats: DoubleStatistics,
}

impl FloatDataPosition {
...
impl FloatDataStreams {
    pub fn position(&self) -> FloatDataPosition {
        FloatDataPosition {
...
impl FloatData {
    ...
    fn finish_row_group(&mut self) {
        if self.row_group_stats.num_values > 0 {
            ...
            self.row_index_entries.push(FloatRowIndexEntry {
    ...
    pub fn write(&mut self, x: f32) {
        ...
        self.streams.data.write_f32::<LittleEndian>(x).unwrap();
        self.row_group_stats.update(x as f64);
        self.check_row_group();
    }
}

impl GenericData for FloatData {
